...
function searchTraceHandler(e) {
  ...
  if (maxDurationValue)
    searchText += "EndTimeUnixNano<=" + maxDurationValue + " ";
  if (minDurationValue)
    searchText += "StartTimeUnixNano>=" + minDurationValue + " ";
  if (tagValue) searchText += tagValue;
  if (searchText == "") searchText = "*";
  else searchText = searchText.trim();
  let queryParams = new URLSearchParams(window.location.search);
  let stDate =
    queryParams.get("startEpoch") || Cookies.get("startEpoch") || "now-3h";
...
function reSort() {
  ...
  for (let i = 0; i < returnResTotal.length; i++) {
    $("#warn-bottom")
      .append(`<div class="warn-box warn-box-${i}"><div class="warn-head">
    ...
    $(`#span-id-head-${i}`).text(
      json.service_name + ": " + json.operation_name + "  "
    );
    ...
    else if (timePass.minutes == 1)
      timePassText = timePass.minutes + " minute ago";
    else if (timePass.minutes > 1)
      timePassText = timePass.minutes + " minutes ago";
